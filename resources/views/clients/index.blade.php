...
<div class="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
    <div class="p-6 border-b border-slate-200 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
            ...
            <p class="text-sm text-slate-500 mt-1">Manage your customer database.</p>
        </div>
        <div x-data="{}" class="flex items-center space-x-2">
            <a href="{{ route('clients.export') }}" class="inline-flex items-center justify-center px-4 py-2 bg-emerald-600 text-white rounded-lg hover:bg-emerald-700 transition-colors text-sm font-medium">
                <i data-lucide="download" class="w-4 h-4 mr-2"></i>
                Export
            </a>
            <button @click="$dispatch('open-modal', 'import-clients')" class="inline-flex items-center justify-center px-4 py-2 bg-slate-600 text-white rounded-lg hover:bg-slate-700 transition-colors text-sm font-medium">
                <i data-lucide="upload" class="w-4 h-4 mr-2"></i>
                Import
            </button>
            <a href="{{ route('clients.create') }}" class="inline-flex items-center justify-center px-4 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition-colors text-sm font-medium">
            ...
            <tbody class="divide-y divide-slate-200 text-sm">
                ...
                <tr class="hover:bg-slate-50 transition-colors">
                    <td class="px-6 py-4">
                        ...
                        <div class="text-slate-500 text-xs mt-0.5 font-mono">NPWP: {{ $client->tin ?? '-' }}</div>
                    </td>
                    <td class="px-6 py-4">
                        <div class="text-slate-800">{{ $client->contact_person ?? '-' }}</div>
                        <div class="text-slate-500 text-xs mt-0.5">{{ $client->email ?? '-' }} | {{ $client->phone ?? '-' }}</div>
                    </td>
                    <td class="px-6 py-4">
                        <div class="text-slate-600 text-xs max-w-xs truncate" title="{{ $client->address }}">{{ $client->address ?? '-' }}</div>
                    </td>
                    <td class="px-6 py-4 text-right">
                        <div class="flex items-center justify-end space-x-3">
                            <a href="{{ route('clients.edit', $client) }}" class="text-slate-400 hover:text-indigo-600 transition-colors" title="{{ __('Edit') }}">
                                <i data-lucide="edit-2" class="w-4 h-4"></i>
                            </a>
                            <form action="{{ route('clients.destroy', $client) }}" method="POST" class="inline-block" onsubmit="return confirm('{{ __('Are you sure?') }}');">
                                @csrf
                                @method('DELETE')
...
<div x-data="{ show: false }" 
     x-show="show" 
     @open-modal.window="if ($event.detail === 'import-clients') show = true"
                    ...
                    <div class="sm:flex sm:items-start">
                        ...
                        <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
                            ...
                            <div class="mt-2 text-sm text-slate-500">
                                <p class="mb-3">Please upload an Excel file (.xlsx, .csv) containing your clients data.</p>
                                <a href="{{ route('clients.template') }}" class="text-indigo-600 hover:text-indigo-900 font-medium flex items-center mb-4">
