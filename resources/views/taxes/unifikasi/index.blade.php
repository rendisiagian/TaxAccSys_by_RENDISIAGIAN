<x-app-layout>
@section('title', 'PPh Unifikasi')

@section('content')
<div class="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
    <div class="p-6 border-b border-slate-200 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
            <h2 class="text-lg font-semibold text-slate-800">PPh Unifikasi</h2>
            <p class="text-sm text-slate-500 mt-1">Manage Bukti Potong PPh 22, 23/26, and 4(2).</p>
        </div>
        <a href="{{ route('taxes.unifikasi.create') }}" class="inline-flex items-center justify-center px-4 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition-colors text-sm font-medium">
        ...
        <table class="w-full text-left border-collapse">
            <thead>
                <tr class="bg-slate-50 text-slate-500 text-xs uppercase tracking-wider border-b border-slate-200">
                    <th class="px-6 py-4 font-semibold">Date & Bupot No</th>
                    <th class="px-6 py-4 font-semibold">Counterpart</th>
                    <th class="px-6 py-4 font-semibold">Tax Type</th>
                    <th class="px-6 py-4 font-semibold text-right">DPP & Amount</th>
                ...
                <tr class="hover:bg-slate-50 transition-colors">
                    ...
                    <td class="px-6 py-4">
                        <span class="inline-flex items-center px-2 py-1 rounded text-xs font-semibold bg-blue-100 text-blue-700 uppercase tracking-wider">
                            {{ str_replace('_', ' ', $trx->tax_type) }}
                        </span>
                        <div class="text-slate-500 text-xs mt-1">Rate: {{ floatval($trx->tax_rate) }}%</div>
                    </td>
                    <td class="px-6 py-4 text-right">
                        <div class="text-slate-500 text-xs">DPP: {{ number_format($trx->tax_base, 2, ',', '.') }}</div>
                        <div class="font-medium text-slate-800 mt-0.5">Rp {{ number_format($trx->tax_amount, 2, ',', '.') }}</div>
                    </td>
